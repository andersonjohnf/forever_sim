...
for (const [label, device] of [
  ...
  test.describe(`Setups (${label})`, () => {
    ...
    test('saves a Fury setup, and loads it back from Arms, switching spec', async ({ page }) => {
      ...
      await expect(loaded).toContainText('You’re on Fury Warrior now.')
    ...
    test('a saved setup that’s out of date loads normalized, and says so', async ({ page }) => {
      ...
      await expect(toast(page, 'Loaded “Old arms”')).toContainText('You’re on Arms Warrior now. One part was out of date and is back to its default.')
      await page.getByRole('tab', { name: 'Character', exact: true }).click()
