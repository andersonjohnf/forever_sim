...
- **Robustness** (seed 36506, 100,000 paired fights each): the new defaults against the old, by
  ...
  ([§6.1](#61-talent-builds)), for a player whose fights are short: it leads in fights up to about 45 s with an execute phase, or up to about 90 s without one (+0.8% at 30 s with a 20% phase, seeds 5151 and 7777; the verification pass VW4-2).
...
23. **Build variants.** ✅ Settled by the sim in W4 ([§6.1](#61-talent-builds)). "Fury + Precision"
    ...
    since W4. Only in short fights (up to about 45 s with an execute phase, 90 s without) does 17/34 still lead (+3.2% at 30 s,
