...
- **Item tooltips, three pre-existing lows (review VT-2, VT-3, VT-4).** Closing the picker with
  ...
  one and the hovered one); the next Escape closes both. While a tooltip is pinned in the phone
  picker, the sheet can't be dragged down until a tap outside closes it. On a phone card the info
