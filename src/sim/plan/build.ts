...
import { Assumptions, BEAR_TEXT, powerInfusionText, preAqRanksText, rogueFinisherApText } from './assumptions'
...
export function buildPlan(config: SimConfig): PlanBundle & { blockers: string[] } {
  ...
  const auraIndex = (spec: AuraSpec, key: string, icon: string) => {
    ...
    auras.push({
      ...
      // buffs doc §1.1 "Power Infusion": an aura that outranks it (Arcane Power, Power Infusion's): [?] placeholder (D24).
  ...
  for (const id of rogueAssumptions(plan, setup.talents, profile.id)) {
    if (id !== 'rogueFinisherAp') notes.add(id)
    else {
      const has = (ability: string) => plan.abilities.some((a) => a.id === ability)
      notes.addText(id, rogueFinisherApText({ eviscerate: has('eviscerate'), rupture: has('rupture'), classicEra: profile.id === 'classicEra' }))
    }
  }
