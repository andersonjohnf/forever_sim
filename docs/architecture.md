...
- **Rotation as a priority list** ([D31](decisions.md#d31-the-rotation-tab-is-an-action-priority-list-you-reorder-2026-09-24)).
  ...
  `rotationApl` in `sim/classes/rotation.ts`; Fury first, then the Feral bear, the rest in M5.65
  A2):
  ...
  - **Presets** (`presets`): an order plus values for the rows' settings. The spec's defaults are
    the implicit `default` preset, named by `defaultPreset` where they're a named rotation (a
    tank's Balanced, D28). `activeAplPreset` says which one the list matches, or `custom`.
    Picking one (`applyAplPreset`) sets its order and row values and resets the rest of the rows'
    settings, keeping the spec-wide ones you set. A spec-wide setting a preset names (a tank's
    Priority, whose value is the preset and which has no control but the picker) is the
    presets' too: picking any preset sets it or puts it back at its default, and every preset is
    compared on it, so two presets whose rows resolve the same (the bear's Balanced and Max TPS)
    stay apart.
