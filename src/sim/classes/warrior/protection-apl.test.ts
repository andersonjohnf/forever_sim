...
// the rows cover every setting, the duties are pinned, the compiler follows the stored order, and
// the three presets and "Custom" read and apply as D28 says.
...
import { activeAplPreset, aplPresets, applyAplPreset, CUSTOM_APL_PRESET, defaultAplOrder, moveAplRow, normalizeAplOrder } from '../apl'
...
describe('Protection’s priority list (D31)', () => {
  it('has a row or a spec-wide place for every setting, and only its own', () => {
    ...
    // The pre-pull first, and the duties where D26's rule puts them, before any threat ability on the GCD.
    expect(PROTECTION_APL.rows.filter((r) => r.pinned).map((r) => r.id)).toEqual(['prepull', 'shieldBlock', 'thunderClap', 'demoShout'])
    expect(defaultAplOrder(PROTECTION_APL).indexOf('demoShout')).toBeLessThan(defaultAplOrder(PROTECTION_APL).indexOf('shieldSlam'))
    // The Priority choice is the preset picker.
    expect(PROTECTION_APL.presetChoice).toBe(PRIORITY)
    expect(PROTECTION_APL.specWide).toContain(PRIORITY)
  })

  it('can’t move a row past a duty, or a duty at all', () => {
    const order = defaultAplOrder(PROTECTION_APL)
    expect(moveAplRow(PROTECTION_APL, order, 'shieldSlam', order.indexOf('thunderClap'))).toBeNull()
    expect(moveAplRow(PROTECTION_APL, order, 'thunderClap', order.indexOf('shieldSlam'))).toBeNull()
    expect(moveAplRow(PROTECTION_APL, order, 'heroicStrike', order.indexOf('bloodrage'))).toBeNull()
    // A stored order that tries it is put back.
    expect(normalizeAplOrder(PROTECTION_APL, ['shieldSlam', 'thunderClap', 'demoShout', 'shieldBlock'])).toEqual(order)
  })

  it('builds the list in the stored order; the consumables stay before the duties', () => {
...
describe('Protection’s presets: Defensive, Balanced and Max TPS (D28)', () => {
  ...
  it('picking one sets its Priority and resets the list’s settings, keeping the spec-wide ones you set', () => {
    ...
    const balanced = applyAplPreset(PROTECTION_APL, saved, 'balanced')!
    expect(activeAplPreset(PROTECTION_APL, PROTECTION_OPTIONS, balanced.rotation, balanced.rotationOrder, TALENTS)).toBe('balanced')
    expect(applyAplPreset(PROTECTION_APL, saved, 'default')).toBeUndefined()
  })
  ...
  it('runs Balanced by default, and Defensive for a setup that chose it, as the presets', () => {
    ...
    expect(plan(base)).toEqual(plan({ ...base, rotation: applyAplPreset(PROTECTION_APL, {}, 'balanced')!.rotation }))
