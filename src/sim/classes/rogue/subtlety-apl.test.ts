...
// list in its default order plays exactly as Subtlety played before it.
import { describe, expect, it } from 'vitest'
import { buildPlan } from '../../plan/build'
import { SUBTLETY_OPTIONS } from './subtlety'
...
describe('the Subtlety rogue’s priority list (D31)', () => {
  it('gives 200 random setups the plan they had before the list', () => {
    ...
    const plans = subtletyCases(SUBTLETY_OPTIONS, 200).map((config) => buildPlan(config).plan)
