...
// checks. The doc's worked examples are in ranged-examples.test.ts.
...
import { addProc, armsPlan, at, counter, damages, line, timeline } from './test-helpers'
 ...
 */
function addShot(plan: Plan, patch: { castMs?: number; cooldownMs?: number; gcdMs?: number; flat?: number; normalized?: boolean; castRangedHasted?: boolean; school?: number; weaponPercent?: number } = {}): number {
...
function addPlanAura(plan: Plan, id: string, durationMs: number, mods: Partial<AuraPlan>): number {
  plan.auras.push({
    id,
    name: id,
    icon: 'x',
...
function addCast(plan: Plan, aura: number, cooldownMs = 600000): number {
...
function addPetAbility(plan: Plan, patch: Partial<PetAbilityPlan> = {}): number {
  const pet = plan.pet!
...
const petLine = (plan: Plan, ability: number, conditions: RotationCondition[] = []) => plan.pet!.rotation.push({ ability, conditions })
