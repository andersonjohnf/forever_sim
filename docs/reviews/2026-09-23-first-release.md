...
| FV2 | low | **iPad with a hardware keyboard:** Enter after typing in the talent paste dialog gives a 10 s toast with no Option+T hint, because touch is detected by `(pointer: coarse)` alone. | superseded, `23fbd9b`: toasts no longer have Undo or a keyboard hint (D21) |
...
| # | Severity | Finding | Disposition |
| --- | --- | --- | --- |
| QV1 | medium, blocking (pre-existing) | **A select opened near the bottom edge lists its options under a waiting toast.** Fight → Advanced → Creature type, Enter, ArrowDown: at 390 px the active option is covered by up to 135 px of its 44; at 1280 px, entirely. Zone too. ux.md says a toast never hides the focused control. The enchant popovers flip above the toast and are fine. | fixed, `334af80` and `4c16a35`: `src/components/select-content.tsx` wraps all four selects. With no toast up it's shadcn's list as before; opened while one is up, it drops from its trigger with the toasts' reach as its bottom collision padding, so it flips above or shortens. While open it keeps its position, and its padding only rises. New e2e tests fail without it (27 and 10 px covered) |
| QV2 | medium (pre-existing) | **Fields with a 0.1 step show float noise:** one press of − on "Whirlwind: Bloodthirst cooldown left" (1.5) shows 1.4000000000000001, and a screen reader reads all of it. 22 of the 61 values from 0 to 6, and Swing speed. Saved setups and links carry the noise; results don't. | fixed, `21a5626`: `snapToStep` rounds to the step's decimals; a unit test walks every 0.1 step from 0 to 6 both ways |
| QV3 | low | **Whole-number fields still read a decimal comma as a thousands separator:** "2,5" in Length variation gives 25%, "1,5" in "Recklessness in the last" 15 s, though "2.5" gives 3. | fixed, `21a5626`: a separator is the decimal point in any field unless it groups thousands, so "2,5" in a whole-number field is 2.5, rounded to 3, as "2.5" is |
| QV4 | low (new in `7f5a8a6`) | **`revealFocus` scrolls after a click when focus is in a text field,** because Chromium matches `:focus-visible` on any focused text input: click Seed while it's partly under a 10 s toast, click Undo, and the page scrolls 32 px. | fixed, `92ec269`: `revealFocus` also needs the last input to have been a key press. New e2e test fails without it (20 px scrolled) |
| QV5 | low (new in `7f5a8a6`) | **Hovering the toasts scrolls the page:** the mouse spreads the stack out, the clearance grows, and keyboard focus is scrolled 22 px, at 1280 px even though the toasts don't overlap it. | fixed, `6f998a9`: focus is revealed only when the settled stack reaches higher, not when the mouse or Alt+T spreads it. New e2e test fails without it (32 px scrolled) |
| QV6 | low | **`--toast-wait-clearance` goes stale after a resize,** because sonner re-measures `--initial-height` only on mount or a content change: 420 → 1280 px leaves the footer link 6 px under the toast. | fixed, `044b615`: the front toast's height is read live, and a resize is measured a frame later, once the toasts sit on the phone's bar (a second cause the fix found: 27 px short and 59 px over). New e2e tests fail without either part |
| QV7 | low (a PV5 trade-off) | **A taller 10 s toast in front of a waiting one covers the last control:** with "Couldn't copy the link" in front at 390 px, the stack reaches 201 px but the padding 169, so the footer link is 21 of its 44 px under it for up to 10 s. | fixed, `e7ef259`: while a waiting toast is up, its room is the highest the settled stack has reached, and only rises until it goes; a new window width starts it afresh. New e2e test fails without it (21 px covered) |
| QV8 | low | **Over-clearance:** About at 1280 px gets 116 px of bottom padding though the toast doesn't overlap the sheet, and the desktop item picker 116 px where about 49 would do. It reads as breathing room. | waived: it's extra room, never too little, and reads as breathing room in the screenshots. Measuring each scroller's own inset would add code to every sheet for no user benefit |
| QV9 | low | **"None goes above 21" has no guard:** a future fractional field with a maximum of 1000 or more would read "1,500" as 1.5. | fixed, `21a5626`: "once before three digits" groups thousands only in a whole-number field or one that reaches 1000, so the rule guards itself rather than resting on today's ranges |
| QV10 | low | **The handoff is stale:** the test counts, and PV1–PV9 "being fixed". | fixed with this log's close: the handoff's counts and status |
...
| --- | --- | --- | --- | --- |
| RV1 | medium, blocking | introduced (`21a5626`) | **Editing a grouped field's own display misreads it.** The field shows "3,000"; Backspace gives "3,00", read as 3 and clamped to 100 fights. "10,000" plus a "0" gives 100 fights; boss armor "3,500" plus Backspace gives 4. | fixed, `6c6a52b` (M2.4e), by a simpler design (user decision): the field shows the plain number while you edit it, so it never reads its own separators |
| RV2 | low | introduced (`334af80`, `4c16a35`) | **An open list moves clear of a growing toast stack, but its active option can end up clipped** 36 of 44 px below the list's scroll edge until the next key press. | superseded, `23fbd9b`: Undo and its waiting toasts are gone (D21, M2.4f) |
| RV3 | low | introduced | **A select's look depends on whether a toast is up,** even a 10 s one: over its trigger without, dropping from it with a slide with one. | superseded, `23fbd9b`: Undo and its waiting toasts are gone (D21, M2.4f) |
| RV4 | low | introduced | **The flip ignores horizontal overlap and the sticky header:** at 1280 px, Zone flips above though the toast isn't under it; at 390 px, a list flipped to the top covers the header and tabs. | superseded, `23fbd9b`: Undo and its waiting toasts are gone (D21, M2.4f) |
| RV5 | low | introduced (the `4c16a35` doc line) | **ux.md says an open list doesn't move when a toast goes,** but at the page's end the padding shrinks, the page scroll clamps, and the list moves 49 or 68 px with its trigger. | superseded, `23fbd9b`: Undo and its waiting toasts are gone (D21, M2.4f) |
| RV6 | low | pre-existing, breaks a ux.md promise | **In landscape on a phone (844×390), the item picker sits under a waiting toast,** which keeps the bar's offset though the picker covers the bar: its controls 30 of 44 px covered, item rows 102 of 103. | superseded, `23fbd9b`: Undo and its waiting toasts are gone (D21, M2.4f) |
| RV7 | low | pre-existing | **`snapToStep` rounds exact halves unevenly** in 0.1 fields ("0,15" to 0.1 but "0,25" to 0.3): float division. | fixed, `6c6a52b` (M2.4e): the quotient is tidied before rounding |
...
## Full review of M2.4e–h (M2.4i)

New work, so full logic and UX reviews by fresh reviewers (D20), over `3eedbb1..b25048d`: the
number fields, the Undo removal (D21), Setups, and rage from damage taken.

**Confirmed (logic):**
- rage from damage taken is `10 × D_pre / maxHealth` per hit as rage.md states: blocks and absorbs give full rage, avoided hits never reach it, and Enrage fires only on a hit that costs health
- only Protection's golden moved (216.97981 → 216.98889). Fury and Arms are unchanged, and the random numbers are consumed exactly as before
- `snapToStep` leaves no float noise over every field's full step chain
- imports: prototype keys can't pollute, and codes, files and configs have size caps. Newer versions are refused, and hidden specs are kept
- `modify()` re-reads storage before it writes, so a second tab's saves survive
- the Undo removal left no dead code
- key tests fail without their fixes

**Confirmed (UX):**
- Setups works by keyboard, with visible focus at both widths and themes
- rename, delete, load, code and file import behave as documented
- no horizontal scroll at 320 or 390 px, whatever the name
- notices go in about 10.6 s and nothing moves when they do
- selects clear a notice at 390, 1280 and 844×390
- number fields show the plain number while editing, and Tab then typing replaces the value
- live regions stay audible under a sheet

Checks: lint ✓ · typecheck ✓ · unit ✓ (1014) · e2e ✓ (190, 3 deferred to M3).
...
| LX4 | low | introduced | **Rage from damage taken is floored to a tenth per hit,** where the logs fit rounding (an 8-damage hit gave +0.3 against 0.26 predicted). Small damage streams lose their rage (10/s: all of it). | fixed, `0e810c3` and `1caec54`: in Forever a white hit's or a hit taken's fraction of a tenth carries to the next (848 swings rule out flooring and rounding each gain); the one-hander constant becomes 3.46 (777 swings, 23 characters; 3.5 can't give the logged 8.9–9.0 on a 2.6 s weapon). Both [?] under D22, with open question 9 |
...
| LX10 | low | introduced | **The "Damage you take" help** doesn't say the number is before mitigation or that it gives rage. | fixed, `dd3adc1` (as UX5) |
| LX11 | low | introduced | **Stale milestone text** (a closed gap, old counts). | fixed, `697c102` |
| LX12 | low | introduced | **`parseNumber` accepts `0x10`, `1e3` and `Infinity`.** | fixed, `5cb6606`: a sign, digits, separators and one decimal point only |
| LX13 | low | pre-existing (tooling) | **`npm run snap` and `test:e2e` share `dist/`**, so running them together breaks the tests. | fixed, `44abefa`: snaps build into `.cache/snap-dist` |
...
| UX5 | medium | introduced | **The "Damage you take" help predates M2.4h:** it doesn't say before armor, or that it gives rage. | fixed, `dd3adc1`: "What the boss deals you per second, before your armor. Each hit gives rage and can trigger Enrage. At 0 you're never hit." |
...
| UX12 | low | introduced | **The damage-taken assumptions are split across three groups,** and the rage one reads awkwardly. | fixed, `1352d7a`: `dpsDamageTaken` sits next to the rage formula; plainer wording |
| UX13 | low | pre-existing, breaks a ux.md promise | **Text field borders are 1.26:1 (light), about 1.5:1 (dark),** against ux.md's 3:1 for controls. | fixed, `a842078`: `--input` gives 3.64:1 (light) and 3.51:1 (dark) on fields, selects, outline buttons and choices; an e2e measures every control type |
| UX14 | low | introduced | **A click in a grouped number field can put the caret one place off,** as the comma goes. | fixed, `4320550`: after a press the field swaps once the browser has placed the caret, then maps it |
| UX15 | low | pre-existing | **Bad number input gives no feedback:** "abc" reverts, 99999 clamps silently. | deferred: pre-existing, breaks no documented promise; listed in the known gaps |

Golden runs after the fixes: Fury 673.8 → 668.6 DPS (409.0 → 405.5 TPS), from the one-hander
constant 3.46; Arms 610.7 → 611.9 DPS, from carried fractions (a 3.5 s two-hander gives 15.75 a
swing); Protection 217.0 TPS. Each change is explained in `src/sim/engine/engine.test.ts`.
