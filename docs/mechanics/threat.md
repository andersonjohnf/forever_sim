...
[wording table](#threat-wording-table) (D29): Lacerate's, Forever's Sunder Armor's client value at
the same level since build 1.60.1.70009, a flat 206 (261, Classic Era's, before), and Shield Slam's
"very high" +475.
...
| --- | --- | --- | --- |
| "causes a high amount of threat" (a special on the GCD) | Sunder Armor r5 (level 58): **261** [C] (Magey); Forever's client **206** [F] (effect 63, 1.60.1.70009; r1–r4 34/75/117/158 at levels 10/22/34/46), plus an attack power term the notes add and the client doesn't carry, **0.05 × AP** [?] (1013 in 1.60.1.69913): about 281 at the default warrior's ~1,500 AP when its Sunders land. Revenge r5 (54): 2.25 × dmg + **243**, r6 (60): 2.25 × dmg + **270** [C]. Shield Slam r4 (60): dmg + **254** [C], when Classic's tooltip said "high" | Classic Era's flat bonus is **4.5 × the spell's level**: 261 = 4.5 × 58, 243 = 4.5 × 54, 270 = 4.5 × 60 (Shield Slam 4.23 × 60). Forever's Sunder is about 3.4–3.55 × its level (34 / 10 … 206 / 58) plus 0.05 × AP, a guessed share that meets Classic's 261 at 1,100 AP and passes it above ([warrior](#warrior)) | **Lacerate** r3 (level 58): **206** per landed application, flat, on top of 1 per damage ([druid bear](#druid-bear), druid.md §4.3, Q15). Rank 3 is Sunder r5's analog: level 58, 15 rage, a 5-stack debuff. Until build 1.60.1.70009 it took Classic's 4.5 × level (r1/r2/r3 189/225/261), since Forever's only value, 1013, was 2.25 × Sunder's armor and so Sunder's alone. 1.60.1.70009's Sunder is 34/75/117/158/**206** at levels 10/22/34/46/58 in the client, level-scaled like the Classic rule: a Forever value for the same words at the same level, which outranks Classic Era's (doctrine §2). Lacerate takes the client value alone, with no attack power term (user decision, 2026-09-26): the notes' "small increase" from attack power has no published size. The sim uses rank 3; a lower rank would take the same 206 as is [?], never a value scaled from it. `classicEra` keeps 261 |
...
| No threat words | Maul and Swipe ×1.75, Faerie Fire 108, Demoralizing Roar 39 ([?], [druid bear](#druid-bear)'s lineage); Thunder Clap ×2.5, Shield Bash 1.5 × dmg + 156, Hamstring 1.25 × dmg + 135, Demoralizing Shout 43.2 [C] | Each ability's own value; one per damage where none is known | Holy Strike, Hammer of the Righteous, Seal of Fury, Judgement of Fury, Consecration, Hammer of Wrath and damage shields (Thorns, Retribution Aura): dmg × 1, × Righteous Fury on Holy. Primal Bite has no words either; its ×1.5 is Blizzard's for the Season of Discovery spell it is ([druid bear](#druid-bear)) |
...
| --- | --- | --- | --- |
| **Sunder Armor** (r5, 11597) | **261** flat per application, even at 5 stacks [C] (Magey) | **206 + 0.05 × AP**. The 206 is [F]; the attack power term is [?]. | Forever's client has an explicit threat effect (effect 63) by rank: r1 **34**, r2 75, r3 117, r4 158, r5 **206** [client] (SpellEffect, 1.60.1.70009). Build 1.60.1.69913 had 1 / 405 / 608 / 810 / 1013 (2.25 × the armor removed); Blizzard's notes for 1.60.1.70009: "Sunder Armor: Corrected the threat values on all ranks, including a small increase to threat generated from Attack Power" ([notes](https://us.forums.blizzard.com/en/wow/t/wow-forever-beta-development-notes-%E2%80%93-updated-september-24/2360696)). The Classic client has no such effect ([client] (SpellEffect, 1.15.9.69722)). The attack power term isn't in the client: the effect has no bonus coefficient (`BonusCoefficientFromAP` and `EffectBonusCoefficient` 0), and no Forever or Classic Era ability has an attack power threat term to copy. **Default: 0.05 × AP** [?] (D29), a reasoned guess rather than a derived share: the notes call the attack power increase small and the change a correction, so the default keeps the total near Classic Era's 261, the closest allowed value for it. 0.05 makes 261 at 1,100 AP, a round figure between the default tank's 802 without raid buffs and 1,241 with them at the pull, and **about 281 at the default tank's ~1,500 AP when its Sunders land** (1,494 on average over a fight, as its procs and cooldowns add to the pull's; +8% over 261), before the stance. The result lists it; warrior.md Q1 and [open-questions B10](../open-questions.md#b10-sunder-armor-threat) hold the test. **Engine:** (the profile's bonus + its AP share × the attack power as it lands) × the global multipliers per landed application (a hit or a block): 206 + 0.05 × AP in `forever`, Classic Era's 261 flat in `classicEra` (`sunderArmor(profile)`; worked examples T1 and T2); none for a miss, dodge or parry. It deals no damage and can't crit ([warrior.md §7](../classes/warrior.md#7-implementation-notes)). |
| Heroic Strike (r8, 11567; +138 dmg), the trainer's ([D36](../decisions.md#d36-what-we-take-from-warriorsim-2026-09-25): r9 is an Ahn'Qiraj book) | dmg + **145** [C] (Magey). r9 (25286, +157): dmg + 173. | [?] | Tooltip unchanged ("high amount of threat"). LTC2 uses 175 for r9. **Engine:** dmg + 145. |
...
| **Primal Bite** (Forever talent; 407995 / 1238069 / 1238070 / 1238073; Mangle (Bear) until 1.60.1.70009) | — | **1.5 × dmg** [?] | 20 rage, 6 s CD, 100% weapon damage + 77 at level 60 [F]. Hits 3 targets under Berserk. Build 1.60.1.70009 renamed it (its debuff, which the client never carried, was removed) and kept every value; its tooltip names no threat ("Bite the target for $s2% normal damage plus $s1", [client] (Spell, 1.60.1.70009)), so the wording table adds nothing. Its talent spell, 407995, is Season of Discovery's Mangle (Bear), carried in the Forever client in place of a Classic spell, and Blizzard's SoD hotfixes of 2 December 2024 say "Mangle (Bear) Threat modifier increased to 1.5x threat per damage (was 1.0x threat per damage)" ([SoD hotfixes](https://us.forums.blizzard.com/en/wow/t/season-of-discovery-hotfixes/1788040), the first post's Druid section): the closest analog for how the server runs it (D36, D37). It was dmg × 1 until 2026-09-26: ×1.5 moves the default bear +7.1% TPS. |
...
| Lacerate, Primal Bite (Mangle (Bear) until 1.60.1.70009) | — | New. Lacerate's "high amount of threat": +206 (Forever's Sunder Armor r5's client value) by the [wording table](#threat-wording-table); Primal Bite 1.5 × dmg (Blizzard's for Season of Discovery's Mangle (Bear), the same spell) | [F] spells, [?] threat |
...
interface ThreatEvent {
  ...
  abilityBonus: number;       // e.g. 206 + 0.05 × AP Sunder (Forever), 145 Heroic Strike r8
...
4. **Bear** [?]: Maul and Swipe multipliers (1.75), Demoralizing Roar (39) and Faerie Fire (108):
   ...
   1.60.1.70009); Season of Discovery's Lacerate, the same spell, has Blizzard's 3.33 × damage, an
   allowed analog not adopted ([druid bear](#druid-bear)). **Primal Bite**'s (Mangle (Bear) until
   1.60.1.70009) threat is 1.5 × dmg, Blizzard's for Season of Discovery's Mangle (Bear), the same
   spell. Neither is measured in Forever. Test: a first Lacerate on a fresh mob
