...
describe('worked example 8: mana', () => {
  ...
  it('the default setup: 4,735 mana, 54 Spirit regeneration a tick (Spirit 195), 50% of it while casting (Mindfulness 3/3), 26 mp5', () => {
    expect([bundle.sheet.mana, bundle.sheet.spirit]).toEqual([4735, 195])
    expect(bundle.plan.mana).toEqual({ maxTenths: 47350, regenTickTenths: 540, fiveSecondRuleMs: 5000, mp5TickTenths: 260, inFsrShare: 0.5 })
  })
  ...
  it('downranks with the defaults: rank 10 with Clearcasting or at 10% mana (473.5) or more, rank 4 below', () => {
    const p = bundle.plan
    ...
    expect(lines).toEqual([
      ...
      ['lightningBolt', [{ code: COND.minMana, a: 4735, b: 0 }]],
      ['lightningBoltRank4', []],
    ])
  })
})
...
describe('the Elemental priority list (shaman.md "Elemental priority")', () => {
  it('builds the defaults in order: racial, Mana Tide at 3,000 missing, the potion, Flame Shock, Lava Burst, Chain Lightning with Clearcasting, Lightning Bolt', () => {
    ...
    expect(tide.conditions).toEqual([{ code: COND.maxMana, a: 47350 - 30000, b: 0 }])
  ...
  it('never swings: the plan has no weapon, though the weapon’s stats count', () => {
    ...
    // Mindfang's 94 spell power.
    expect(bundle.sheet.spell!.caster!.schoolDamage.nature - unarmed.sheet.spell!.caster!.schoolDamage.nature).toBe(94)
