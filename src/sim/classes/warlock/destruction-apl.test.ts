// The Destruction warlock's rotation as a priority list (decision D31; docs/classes/warlock.md §6.1).
import { describe, expect, it } from 'vitest'
import { buildPlan } from '../../plan/build'
import { destructionCases } from './destruction-apl-cases'
import { fingerprint, planJson } from './apl-cases'
import { DESTRUCTION_OPTIONS } from './destruction'
...
describe('the Destruction warlock’s priority list (D31)', () => {
  it('gives 200 random setups the plan they had before the list', () => {
    ...
    const plans = destructionCases(DESTRUCTION_OPTIONS, 200).map((config) => buildPlan(config).plan)
