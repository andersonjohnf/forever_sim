...
src/
  ...
  sim/             the engine: pure TypeScript, no React/DOM; index.ts is the public API
    ...
    optimize/      the optimizer (docs/optimizer.md): racing on common random numbers, the talent
                   screen and space, constraints, per-fight runners
  ...
  app/             the app shell: header, sheets, persistence, sharing, notices; releases.ts is the
                   hand-written release notes and roadmap.ts what's coming (Deployment, "Release notes")
scripts/scrape/    scrapers: client files via wago.tools → src/data (Node ≥22, zero dependencies)
scripts/tune/      the rotation tuner and the optimizer's CLI, on an engine bundled from src/
...
- **A part follows the default while it holds it.** A gear slot follows while its item and enchant
  ...
  player's own too (`defaultOffHandBeside`, one rule: [ux.md "Gear"](ux.md#sections)); the talent build
...
- **A player's own talent build that lost points on the game's new trees** (a version-1 code mapped
  by name), **or a build the sim shipped read as its successor** (`talentChange` from
...
- **Coming soon** (`src/app/coming-soon-sheet.tsx`) lists what's coming rather than what shipped.
  `src/app/roadmap.ts` is its source: a hand-written list of the agreed milestones from
  `docs/milestones.md` for players, the next first. It shares Release history's sheet and entry
  layout (`src/app/notes-sheet.tsx`), and `src/app/roadmap.test.ts` holds it to the release notes'
  player-voice rules (`src/app/player-voice.ts`) and to its order: Next update, then Planned, then Later.
