// The committed talent datasets against freshly generated ones (usually a new client build),
// talent by talent: position, max rank, prerequisite, icon, per-rank Forever and Classic texts,
// the Classic match, change kind, passive flag and tooltip header, plus what each stored build
// code decodes to. Writes <outDir>/talents-diff.md and .json and prints a summary. Also writes
// <outDir>/talents-changes.md, the new dataset's own Forever-vs-Classic tables
// (docs/data/talents.md#changes-at-a-glance). Run through `npm run diff:talents`
// (scripts/scrape/talents-client.mjs --diff).
...
function talentChanges(o, n, oldById, newById) {
  ...
  for (const f of ["id", "name", "tree", "tier", "col", "order", "maxRank", "icon", "changeKind", "passive", "previousName", "spellId", "classicSpellId"]) cmp(f, o[f], n[f]);
  ...
  for (const k of ["name", "tree", "tier", "col", "maxRank", "matchStatus", "prerequisite"]) cmp(`classic.${k}`, o.classic?.[k], n.classic?.[k]);
...
export function diffTalents({ pairs, against, outDir, repoRoot, decodeByName, repoCodes }) {
  const report = { against, classes: {}, byField: {} };
  const md = [
    "# Talents: committed dataset vs fresh generation",
    "",
    `Generated by \`npm run diff:talents\` against ${against}. Talents are paired by tree and name, then by cell.`,
    "",
  ];
  for (const { cls, old, next } of pairs) {
    ...
    const decode = (data, code) => {
      try {
        return decodeByName(data, code);
      } catch (e) {
        return { error: e.message };
      }
    };
    const codes = (repoCodes[cls] ?? []).map((code) => {
      const a = decode(old, code);
      const b = decode(next, code);
    ...
    md.push("| Stored code | Same ranks by name |", "| --- | --- |");
      ...
      for (const c of p.changes) {
        if (c.field.startsWith("ranks.")) {
          ...
          for (let i = 0; i < len; i++) {
            ...
            md.push(`  - rank ${i + 1} committed: ${json(c.old?.[i] ?? null)}`, `  - rank ${i + 1} new: ${json(c.new?.[i] ?? null)}`);
          }
  ...
  console.log(`\nTalents diff against ${against} (${path.relative(repoRoot, path.join(outDir, "talents-diff.md"))}):`);
  for (const [cls, r] of Object.entries(report.classes)) {
    console.log(`  ${cls}: ${r.counts.old} → ${r.counts.new} talents, ${r.counts.changed} changed, ${r.counts.added} added, ${r.counts.removed} removed; stored codes ${r.codes.filter((c) => c.same).length}/${r.codes.length} same`);
