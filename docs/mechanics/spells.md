...
| **Power Infusion** (10060) | +20% spell damage, 15 s, 3 min cooldown, off the GCD, unchanged | the same | Buffs entry, the casters' and the Protection paladin's, cast on you once at the pull (an Arcane mage's as its Arcane Power ends): a caster's rotation presses it ([§12](#12-what-a-class-slice-uses); [buffs](buffs-debuffs-consumables.md#power-infusion)) | [F] [C] [client] (SpellEffect, SpellCooldowns, both builds) |
...
- **Top-rank base damage is lower, the coefficients the same or higher.** At level 60, before
  ...
  | Lightning Bolt (10) | 189–211, **2.5 s** | 429–477, 3.0 s | **0.714** / 0.857 |
...
- **DoTs** have one slot per plan spell: ticks left, generation, next tick, snapshot damage and
  ...
  the last tick.
- **Damage procs and the rogue's poisons** read the same school numbers as the spell table: a
