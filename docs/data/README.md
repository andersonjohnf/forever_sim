...
item pool and `src/data/client` are read from the WoW Forever and Classic Era client files
through the [wago.tools API](https://wago.tools/apis) (D16). Spells, talents and races still
come from [foreverchanges.pro](https://foreverchanges.pro), which datamines the Forever beta
client and diffs it against Classic Era, until they move to the client files too
...
The client datasets (`src/data/client/*`, `src/data/items/pre-bis.json`) carry the source
endpoint, product, build, the tables read with their FileDataIDs, the WoWDBDefs commit and
`scrapedAt` instead ([client.md](client.md)). The About sheet lists every dataset with its
build and date.
...
```sh
npm run scrape                # spells, talents, races (foreverchanges) and items (client), using the cache
