...
test.describe('Protection paladin', () => {
  test('is in the spec switcher under Paladin, with its default build in the Talents preset menu and About naming it', async ({ page }) => {
    ...
    await expect(page.getByRole('dialog').getByText('Covers Warriors: Fury, Arms and Protection · Druids: Feral (Cat) and Feral (Bear) · Paladins: Retribution and Protection · Shamans: Enhancement · Rogues: Combat, Assassination and Subtlety · Mages: Fire, Frost and Arcane · Warlocks: Destruction and Affliction.', { exact: true })).toBeVisible()
  })
