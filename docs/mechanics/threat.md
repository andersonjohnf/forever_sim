...
Classic Era numbers and flags them.
...
- The values stay, but **a person should confirm each one in a browser** before treating it as
  settled. The most important is Sunder Armor's threat effect of 1013.
- Never fetch wago.tools from scripts or agents.
- Where a foreverchanges.pro tooltip gives the same fact, the tooltip is the primary citation.
...
- **Ability multiplier and flat bonus** are per ability ([tables](#per-ability-threat-at-max-rank)).
  ...
  (aura 108, modifier 2). [F] DB2 [1311034](https://wago.tools/db2/SpellEffect?build=1.60.1.69913), 23561.
...
- **Exception (a flat bonus on the same aura).** Classic Feral Instinct did not add its own threat
  aura. It added +3 per rank to Bear Form (Passive2)'s +30%, so bear threat was
  1.30 + 0.15 = 1.45, **additive**. [C] DB2 1.15.9: spell 16947 is aura 107 on class mask
  0x2000000 = spell 21178 ([SpellClassOptions](https://wago.tools/db2/SpellClassOptions?build=1.15.9.69722)).
  This no longer applies in Forever (see below).
- **Holy-only multipliers** (Righteous Fury) apply only to Holy-school events. An event's school
  is the spell's school: Holy Strike is all Holy even though it is a weapon strike (DB2 SpellMisc
  school 2).
...
| --- | --- | --- | --- | --- |
| Battle Stance | ×0.8 | all | [F][C] | DB2 21156 aura 10 = −20 ([F](https://wago.tools/db2/SpellEffect?build=1.60.1.69913) and [C](https://wago.tools/db2/SpellEffect?build=1.15.9.69722)); Magey stance tests (0.78–0.82) |
| Berserker Stance | ×0.8 | all | [F][C] | DB2 7381 aura 10 = −20 |
| Defensive Stance | ×1.3 | all | [F][C] | DB2 7376 aura 10 = +30. Forever tooltip "Increases all threat generated by 30%" ([class/warrior](https://foreverchanges.pro/class/warrior)). Magey tests: 1.28–1.31. |
| Defiance, Classic (5 ranks) | ×(1 + 0.03 × rank) in Defensive Stance: ×1.15 at 5/5 | all | [C] | Classic tooltip; DB2 12303…12792 |
| **Defiance, Forever (3 ranks)** | ×(1 + 0.05 × rank) **in Defensive Stance with a shield equipped**: ×1.15 at 3/3 | all | [F] | Tooltip; DB2 12792 aura 10, curve 5/10/15 ([CurvePoint](https://wago.tools/db2/CurvePoint?build=1.60.1.69913)) |
| Bear / Dire Bear Form | ×1.3 | all | [F][C] | DB2 21178 "Bear Form (Passive2)" aura 10 = +30 in both builds |
...
| **Feral Instinct, Forever** | **No threat effect.** It is now +10% Swipe damage per rank plus stealth. | — | [F] | [class/druid](https://foreverchanges.pro/class/druid); DB2 16947 now modifies Prowl and Swipe |
| Cat Form | ×0.71 | all | [F][C] | DB2 3025 aura 10 = −29 |
...
| --- | --- | --- | --- | --- |
| Righteous Fury (spell 25780, 30 min) | Holy threat +60% (×1.6) | **Holy threat +90% (×1.9)** | [F][C] | DB2 25780 aura 10, school mask 2: 59+1 (C) vs 90 (F); [spellbook](https://foreverchanges.pro/spellbook/paladin) |
| Improved Righteous Fury (3 ranks) | +16 / +33 / +50% of RF's bonus: ×1.696 / ×1.798 / **×1.9** | **Threat part removed.** Now 2 / 4 / 6% less damage taken while RF is active. | [F][C] | DB2 20468: Classic spell mod on RF; Forever flat −2/−4/−6 on RF's damage-taken effect (curve 82954) |
| Instrument of Law (Ret, 2 ranks, new) | — | −10% / −20% **all** threat while RF is **not** active | [F] | DB2 1311085 aura 10, curve 10/20; [class/paladin](https://foreverchanges.pro/class/paladin) |
| Iron Creed (Prot, 5 ranks, new) | — | Holy Strike threat +5% per rank (+25% at 5/5). With RF active, Holy Strike also gives 2–10% less damage taken for 6 s. | [F] | DB2 1311034 aura 108, modifier 2 on Holy Strike's class mask, curve 5…25 |
...
- Forever's Holy Strike and Hammer of the Righteous (both Holy school in DB2);
...
| --- | --- | --- | --- |
| Blessing of Salvation / Greater Blessing of Salvation | ×0.7 (all) | [F][C] | DB2 1038 / 25895 aura 10 = −30. Forever duration 1 h ([spellbook](https://foreverchanges.pro/spellbook/paladin)). |
...
| Enchant Gloves – Threat | ×1.02 | [F][C] | [SpellItemEnchantment 2613](https://wago.tools/db2/SpellItemEnchantment?build=1.60.1.69913) → spell 25063 (+2) |
| Enchant Cloak – Subtlety | ×0.98 | [F][C] | SpellItemEnchantment 2621 → spell 25070 (−2) |
| Fetish of the Sand Reaver (use: Arcane Shroud) | ×0.3 for 20 s, 3 min CD | [F][C] | DB2 26400 aura 10 = −70, duration 20 s; ItemEffect CD 180 s ([ItemEffect](https://wago.tools/db2/ItemEffect?build=1.60.1.69913)). LTC2 adds level scaling above 60 (not relevant). |
| The Eye of Diminution (use) | ×0.65 for 20 s, 2 min CD | [F][C] | DB2 28862 = −35, 20 s, CD 120 s. (Spell 1219503, −70%, belongs to a different item, 236302, and is not used here.) |
| **Forever threat equip effects (new)** | ±1% to ±4% | [F] (item data pending) | New spells "Increase Threat All 01–04" (+1…+4) and "Decrease Threat All 01–04" (−1…−4), linked by [ItemXItemEffect](https://wago.tools/db2/ItemXItemEffect?build=1.60.1.69913). One confirmed: *Treacherous Treads* (273842), "Equip: Increases all threat generated by 1%" ([item page](https://foreverchanges.pro/item/273842)). Other links: 278540 (+1), 279493 (+2), 278929 (−2), 278299 (−4), and reworked dungeon items 14576 and 13959 (−1) and 18308 (−4), whose Forever tooltips aren't published yet. Item data belongs to [buffs-debuffs-consumables.md](buffs-debuffs-consumables.md) and the item dataset. The engine only needs a generic `threatPct` equip stat. |
| Enhanced Sunder Armor (Classic T1 "Might" 8-piece) | Sunder threat ×1.15 (ability-specific) | [C] | DB2 23561 exists in both builds. Forever's T1 set bonuses were redesigned (see the items doc), so assume it is absent in Forever. |
...
| --- | --- | --- | --- |
| **Sunder Armor** (r5, 11597) | **261** flat per application, even at 5 stacks [C] (Magey) | **1013** flat. The client value is [F]; the in-game total is [?]. | Forever DB2 adds an explicit threat effect (effect 63) by rank: r1 **1**, r2 405, r3 608, r4 810, r5 1013. That is **2.25 × the armor removed** (180/270/360/450), as [warrior.md §1 and Q1](../classes/warrior.md#9-open-questions) notes. The Classic client has no such effect. Sources: [DB2](https://wago.tools/db2/SpellEffect?build=1.60.1.69913); also read by [ElliotWood/Forever](https://github.com/ElliotWood/Forever/blob/master/docs/beta-pass/warrior.md). **Default: 1013 replaces Classic's server-side 261.** warrior.md Q1 asks whether it is instead added on top (1274). Rank 1 = 1 looks like a bug; 2.25 × 90 would be ~203. Players report low-rank Sunder "generating 10 threat instead of 100" ([forum](https://us.forums.blizzard.com/en/wow/t/warrior-rage-normalization-auto-attack-crits-dont-generate-extra-rage/2355684)). |
| Heroic Strike (r9, 25286; +157 dmg) | dmg + **173** [C] (Magey). r8 (11567): dmg + 145. | [?] | Tooltip unchanged ("high amount of threat"). LTC2 uses 175. |
| Revenge (r6, 25288) | **2.25 × dmg + 270** [C] (Magey). r5: 2.25 × dmg + 243. | [?] | Forever damage rose from 81–99 to **138–168** [F], so with the Classic formula Revenge threat rises a lot. LTC2's older code uses a flat 355, which the measurements supersede. |
| Shield Slam (r4, 23925) | dmg + **254** [C] (Magey) | [?] | Forever damage 640–670 + block value [F] (Classic 342–358). The tooltip changed from "a high amount of threat" to "**a very high** amount of threat", so the bonus may have risen. |
...
| Execute (r5, 20662) | dmg × 1.25? — **unverified** [?] | [?] | Only in [LTC2 Warrior.lua](https://github.com/dfherr/LibThreatClassic2/blob/master/ClassModules/Classic/Warrior.lua) code. Default: ×1.25. |
...
| **Lacerate** (Forever, r3 1235827, trained 42/50/58) | — | **unknown** [?]. Default: dmg × 1 on the hit and the ticks, no bonus. | 15 rage; bleeds 75 over 15 s, stacks to 5 [F]. The tooltip says "Causes a high amount of threat", so a flat bonus almost certainly exists. **The only numbers we found are Season of Discovery values. They are not adopted.** |
| Cower (r3, 9892; cat) | −600 − 1 per level (−608 at 60) [C] | **−1200 − 1 per level (−1208 at 60)** [F] | DB2 effect 63 with a per-level term. DPS context; details in [druid.md §3](../classes/druid.md). |
...
| --- | --- | --- | --- |
| Holy Shield (r3, 20928) | Holy dmg × **1.2** per block, then × RF [C] | Same rule [F tooltip] | "Damage caused by Holy Shield causes 20% additional threat" appears in both clients. Forever r3 deals **221** per block with a 0.08 coefficient and +20% block chance (Classic: 130, 0.05, +30%) [F]. The r2 and r3 Forever tooltips drop "4 charges", which is for [classes/paladin.md](../classes/paladin.md) to resolve. LTC2: ×1.2. |
...
| --- | --- | --- | --- |
| Taunt (warrior), Growl (bear) | If the target isn't already attacking you: set your threat equal to the highest threat on its list, and force it to attack you for **3 s**. Can be resisted (spell hit roll). **8 s CD** in Forever (Classic 10 s). | [F][C] | DB2 355 / 6795: effect 114 (attack me) + aura 11 (taunt), duration 3 s [F]; behaviour per [LTC2 `Taunt`](https://github.com/dfherr/LibThreatClassic2/blob/master/ClassModules/Classic/Warrior.lua) [C] |
| Mocking Blow | Forced attack for 6 s. **No** threat matching. Only its own (unknown) threat. | [F][C] | DB2 20560: aura 11 only, 6 s |
| Challenging Shout / Challenging Roar | Forced attack for 6 s on all enemies in range. No threat change. | [F][C] | DB2 1161 / 5209, 6 s |
...
| --- | --- | --- | --- |
| Sunder Armor threat (r5) | 261 (server) | **1013** (client effect 63); r1 = 1 (bug?) | [F] DB2 |
...
| Lacerate, Mangle (Bear) | — | New; threat unknown | [F] spells, [?] threat |
