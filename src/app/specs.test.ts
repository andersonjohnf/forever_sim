...
describe('the About sheet and page description', () => {
  ...
  test('list the specs one class at a time', () => {
    ...
    // Protection paladin since C3, Enhancement since S1, the three rogues since R1, the mages since K2, the warlocks since K3 (Demonology since H3), the Shadow Priest since K4, Elemental since K5, Balance since K6 and the hunters since H2.
    expect(coverageSentence()).toBe('Covers Warriors: Fury, Arms and Protection · Druids: Feral (Cat), Feral (Bear) and Balance · Paladins: Retribution and Protection · Shamans: Enhancement and Elemental · Rogues: Combat, Assassination and Subtlety · Mages: Fire, Frost and Arcane · Warlocks: Destruction, Affliction and Demonology · Priests: Shadow · Hunters: Marksmanship, Beast\u00a0Mastery and Survival.')
    const all = coverageSentence(specs)
    expect(all).toMatch(/^Covers Warriors: [^·]+ · Druids: [^·]+ · Paladins: [^·]+ · Shamans: [^·]+ · Rogues: [^·]+ · Mages: [^·]+ · Warlocks: [^·]+ · Priests: [^·]+ · Hunters: [^·]+\.$/)
