...
[wording table](#threat-wording-table) (D29): Lacerate's is Forever's Sunder Armor's client value at
the same level since build 1.60.1.70009, a flat 206 (261, Classic Era's, before). Shield Slam's "very
high" has no value in any allowed source, so it keeps Classic Era's bonus, 254 [?] (Classic Era's
[C]), and the extra is 0 [?], an open question.
...
| --- | --- | --- | --- |
| "causes a high amount of threat" (a special on the GCD) | Sunder Armor r5 (level 58): **261** [C] (Magey, 1.13.6); Forever's client **206** [F] (effect 63, 1.60.1.70009; r1–r4 34/75/117/158 at levels 10/22/34/46; 1013 in 1.60.1.69913), flat: the notes add an attack power term that no client carries and nobody has measured, so it has no value and the sim adds 0 [?] (OQ 2). Revenge r5: 2.25 × dmg + **243** [C] (Resultsmayvary, 1.13.2), r6: 2.25 × dmg + **270** [C] (Magey, 1.13.6). Shield Slam r4: dmg + **254** [C] (both series), when Classic's tooltip said "high" | None: each ability and rank has its own measured value, and one ability's value is never derived from another's by a ratio or a per-level rule (D37). A new ability with the words takes a known value as is | **Lacerate** r3 (level 58): **206** per landed application, flat, on top of 1 per damage ([druid bear](#druid-bear), druid.md §4.3, Q15). Rank 3 is Sunder r5's analog: level 58, 15 rage, a 5-stack debuff. Until build 1.60.1.70009 it took Classic's 4.5 × level (r1/r2/r3 189/225/261), since Forever's only value, 1013, was 2.25 × Sunder's armor and so Sunder's alone. 1.60.1.70009's Sunder is 34/75/117/158/**206** at levels 10/22/34/46/58 in the client, level-scaled like the Classic rule: a Forever value for the same words at the same level, which outranks Classic Era's (doctrine §2). Lacerate takes the client value alone, with no attack power term (user decision, 2026-09-26): the notes' "small increase" from attack power has no published size. The sim uses rank 3; a lower rank would take the same 206 as is [?], never a value scaled from it. `classicEra` keeps 261 |
...
| No threat words | Maul and Swipe ×1.75, Faerie Fire 108, Demoralizing Roar 39 ([?], [druid bear](#druid-bear)'s lineage); Thunder Clap ×2.5, Shield Bash 1.5 × dmg + 156, Hamstring 1.25 × dmg + 135, Demoralizing Shout 43.2 [C] | Each ability's own value; one per damage where none is known | Primal Bite, Holy Strike, Hammer of the Righteous, Seal of Fury, Judgement of Fury, Consecration, Hammer of Wrath and damage shields (Thorns, Retribution Aura): dmg × 1, × Righteous Fury on Holy |
...
it takes **2 × its damage** [?] instead, **by name, as a user decision ([D38](../decisions.md#d38-the-values-audits-calls-2026-09-26), 2026-09-26)**: the value
...
- Classic values come from two test series that Magey's table collects. Magey's own threat-API
  tests on 1.13.6.37497 fit `(mult × dmg + bonus) × stance` to within ±0.01 over 7–16 data points
  ...
  unless a row says otherwise. Resultsmayvary's earlier series on 1.13.2
  ([spreadsheet](https://docs.google.com/spreadsheets/d/1z_I0oU5IOmQA5RQX26xOvj3ycLRMTVoZapFLY7OG-8c))
  is an aggro-rip threshold test, not the threat API: it reads the threat from where aggro is
  pulled (at 1.1 or 1.3 × the tank's). It measured **Heroic Strike r8 (+145)** and **Revenge r5
  (+243)**, the ranks the sim uses, and Shield Slam r1–r4 again (the same values). Revenge r5 rests
  on 8 rows, all within ±1. Heroic Strike r8's sample is thin: one tight row (239 damage, ripped
  between 337 and 338 at 0.8 × 1.1, so +144 to +145) and one loose one (2 casts, ±4). Where LTC2's
  older code disagrees, **the measured value wins**.
...
| --- | --- | --- | --- |
| **Sunder Armor** (r5, 11597) | **261** flat per application, even at 5 stacks [C] (Magey) | **206** flat [F]; attack power term 0 [?] (OQ 2) | Forever's client has an explicit threat effect (effect 63) by rank: r1 **34**, r2 75, r3 117, r4 158, r5 **206** [client] (SpellEffect, 1.60.1.70009). Build 1.60.1.69913 had 1 / 405 / 608 / 810 / 1013 (2.25 × the armor removed); Blizzard's notes for 1.60.1.70009: "Sunder Armor: Corrected the threat values on all ranks, including a small increase to threat generated from Attack Power" ([notes](https://us.forums.blizzard.com/en/wow/t/wow-forever-beta-development-notes-%E2%80%93-updated-september-24/2360696)). The Classic client has no such effect ([client] (SpellEffect, 1.15.9.69722)). **The attack power term has no value:** the effect has no bonus coefficient (`BonusCoefficientFromAP` and `EffectBonusCoefficient` 0), no Forever or Classic Era ability has an attack power threat term, and nobody has measured it, so the sim adds none (0 [?]) and it's an open question ([OQ 2](#open-questions), warrior.md Q1, [open-questions B10](../open-questions.md#b10-sunder-armor-threat)). Until 2026-09-26 the sim assumed 0.05 × AP, a share chosen to keep the total near Classic Era's 261; D37 removed it. The result lists the value. **Engine:** the profile's bonus × the global multipliers per landed application (a hit or a block): 206 in `forever`, Classic Era's 261 in `classicEra` (`sunderArmor(profile)`; worked examples T1 and T2); none for a miss, dodge or parry. It deals no damage and can't crit ([warrior.md §7](../classes/warrior.md#7-implementation-notes)). |
| Heroic Strike (r8, 11567; +138 dmg), the trainer's ([D36](../decisions.md#d36-what-we-take-from-warriorsim-2026-09-25): r9 is an Ahn'Qiraj book) | dmg + **145** [C] (Resultsmayvary's 1.13.2 aggro-rip series, a thin sample: [two rows](#per-ability-threat-at-max-rank)). r9 (25286, +157): dmg + 173 (Magey, 1.13.6). | [?] | Tooltip unchanged ("high amount of threat"). LTC2 uses 175 for r9. **Engine:** dmg + 145. |
| Revenge (r5, 11601), the trainer's (D36: r6 is an Ahn'Qiraj book) | **2.25 × dmg + 243** [C] (Resultsmayvary, 1.13.2 aggro-rip series; [method](#per-ability-threat-at-max-rank)). r6 (25288): 2.25 × dmg + 270 (Magey). | [?] | Forever damage rose from 64–78 to **109–133** at r5 (r6: 81–99 to 138–168) [F], so with the Classic formula Revenge threat rises a lot. LTC2's older code uses a flat 355, which the measurements supersede. **Engine:** 2.25 × dmg + 243 (r5, `REVENGE.threatBonus`). |
| Shield Slam (r4, 23925) | dmg + **254** [C] (Magey) | dmg + **254** [?] (Classic Era's [C]); "very high" extra 0 [?] (OQ 1) | Forever damage 640–670 + block value [F] (Classic 342–358). The tooltip changed from "a high amount of threat" to "**a very high** amount of threat", and neither client has a threat effect on it (1.60.1.70009 included). No allowed source gives the "very high" bonus a value, so it keeps Classic Era's 254, measured in two independent series: [Magey's](https://docs.google.com/spreadsheets/d/1VLmhdNX_hZcud8Q0n2I1fPv7y8g8BHYhH_cmIkmd_-s) threat-API tests on 1.13.6 (rank 4 over 7 casts, ±0.01) and [Resultsmayvary's](https://docs.google.com/spreadsheets/d/1z_I0oU5IOmQA5RQX26xOvj3ycLRMTVoZapFLY7OG-8c) aggro-rip threshold tests on 1.13.2 ([method](#per-ability-threat-at-max-rank)), both reading ranks 1–4 as 178 / 203 / 229 / **254**. The bigger damage already raises its threat. wowsims' 508 (254 × 2) cites no source and isn't used. The extra is [OQ 1](#open-questions) (warrior.md Q34). Until 2026-09-26 the sim assumed 475, 254 scaled by the damage ratio; D37 removed it. The result lists the value. **Engine:** dmg + 254 in both profiles (`SHIELD_SLAM`). |
...
| **Primal Bite** (Forever talent; 407995 / 1238069 / 1238070 / 1238073; Mangle (Bear) until 1.60.1.70009) | — | **dmg × 1** [?] | 20 rage, 6 s CD, 100% weapon damage + 77 at level 60 [F]. Hits 3 targets under Berserk. Build 1.60.1.70009 renamed it (its debuff, which the client never carried, was removed) and kept every value; its tooltip names no threat ("Bite the target for $s2% normal damage plus $s1", [client] (Spell, 1.60.1.70009)), so the wording table's "no threat words" row: one per damage (user decision, 2026-09-26: nothing in its tooltip says increased threat). Considered and rejected: its talent spell's id, 407995, is Season of Discovery's Mangle (Bear), and Blizzard's SoD hotfixes of 2 December 2024 say "Mangle (Bear) Threat modifier increased to 1.5x threat per damage (was 1.0x threat per damage)" ([SoD hotfixes](https://us.forums.blizzard.com/en/wow/t/season-of-discovery-hotfixes/1788040), the first post's Druid section). That doesn't carry over: Forever reworked the spell (SoD's is 160% weapon damage with a bleed-damage debuff; Forever's is 100% + 26 at rank 1 with none), and ranks 2–4 are new Forever ids. ×1.5 would be about +7% bear TPS. |
| **Lacerate** (Forever, r3 1235827, trained 42/50/58) | — | **dmg × 1 + 206** per landed application [?]; ticks dmg × 1 | 15 rage; bleeds 75 over 15 s, stacks to 5 [F]. The tooltip says "Causes a high amount of threat", with no threat effect in the client, so the bonus is the [wording table](#threat-wording-table)'s: Forever's Sunder Armor r5 at the same level 58, its client value 206 since 1.60.1.70009, flat (user decision, 2026-09-26); Classic Era's rule, 4.5 × level = 261, until that build. **Engine:** 206 × the global multipliers on each landed application (a hit or a block), the first one too, which deals no damage; none for a miss, dodge or parry. `classicEra`: 261. Season of Discovery's Lacerate is the same spell (Forever's rank 1 reuses its id), and Blizzard's SoD hotfixes of 2 December 2024 give its hit and ticks 3.33 × damage (was 3.5) ([SoD hotfixes](https://us.forums.blizzard.com/en/wow/t/season-of-discovery-hotfixes/1788040)): an allowed analog (D36, D37), not adopted. An in-game test (druid.md G1) replaces the default. |
...
| **Seal of Fury** (Forever, r7 20423) | — | Holy dmg (35 + 0.1 × SP per landed melee hit, whatever the weapon) × RF | New tanking seal [F]; the flat 35 is client data and measured in the beta logs ([paladin.md](../classes/paladin.md#the-beta-logs-seal-of-fury)). It also grants an absorb shield with a shield equipped (no threat) |
| **Judgement of Fury** (20414) | — | 146–160 Holy (153.3–166.7 at level 60, + 0.45 × SP) × RF, and **taunts the target for 4 s** [F] | [paladin.md](../classes/paladin.md#threat-paladin-specific) models the taunt like Taunt: it raises you to top threat, and does nothing if you are already there. We use that as the default, but the mechanism is untested [?]. The client carries a scripted value (a dummy effect of 1607 + 42.3 per level, coefficient 0.18, [F] [client] (SpellEffect, 1.60.1.69913)). Judgement of Righteousness has the identical structure in the Classic Era client, and deals only its own damage there (170–186 + 0.5 × SP) [C], and nothing describes the dummy, so the sim gives it none, no damage and no threat (as flat threat it would be +383 TPS; [paladin.md](../classes/paladin.md#seal-of-fury-sof-new-the-protection-seal); in-game test T5 in its open questions). |
| **Holy Strike** (Forever, r8 10333; half of a normalized weapon swing + 81–105 + 0.429 × SP, as the beta logs show; 10 s since 1.60.1.70009) | — | dmg × RF × (1 + 0.05 × Iron Creed rank) | The whole strike is Holy school (SpellMisc 2) [F]. Its third effect (77, a script) is Sacred Arbiter's judgement refresh, the same effect Judgement 20271 carries, not a threat bonus: its tooltip has no threat wording, so no bonus (D29) [?] (in-game test T4 in [paladin.md](../classes/paladin.md#open-questions)) |
| **Hammer of the Righteous** (Forever, 407632) | — | Holy dmg (3 × main-hand weapon DPS: the weapon's own, as its tooltip reads, by default; attack power counted as a Character → Advanced setting [?]) to up to 4 targets × RF | Holy school [F]. No threat wording in its tooltip, so no bonus (D29). The sim hits one target ([paladin.md](../classes/paladin.md#other-abilities)) |
...
| --- | --- | --- | --- |
| Sunder Armor threat (r5) | 261 (server) | **206** (client effect 63; r1–r4 34/75/117/158); 1013 in 1.60.1.69913. The notes' attack power term has no value: 0 [?] (OQ 2) | [F] [client] (SpellEffect, 1.60.1.70009); attack power term 0 [?] |
...
| Shield Slam tooltip | "a high amount of threat" | "a **very high** amount of threat", with no value: the sim keeps Classic Era's dmg + 254 [?] and adds 0 [?] for "very high" ([wording table](#threat-wording-table), OQ 1) | [F] text; value [?] |
...
| Lacerate, Primal Bite (Mangle (Bear) until 1.60.1.70009) | — | New. Lacerate's "high amount of threat": +206 (Forever's Sunder Armor r5's client value) by the [wording table](#threat-wording-table); Primal Bite unknown (dmg × 1, its tooltip naming no threat) | [F] spells, [?] threat |
...
| T9b | Forever bear (×1.3), 1,200 AP; Lacerate r3 hits for 154.566 (+206 [?], the wording table, whatever the attack power) | (154.566 + 206) × 1.3 = **468.735**; a first application (no damage) **267.8**. `classicEra`'s +261: 540.235 and 339.3 |
...
4. **Bear** [?]: Maul and Swipe multipliers (1.75), Demoralizing Roar (39) and Faerie Fire (108):
   ...
   measured in Forever. Test: a first Lacerate on a fresh mob
