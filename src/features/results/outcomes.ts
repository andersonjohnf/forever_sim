...
export interface OutcomeLines {
  ...
  parts: string[]
  /** Its average damage per landed hit (or tick), shown at the end of the first line; null when it has none to show. */
  average: { value: string; per: 'hit' | 'tick' } | null
 ...
 * fight", "14.2 procs a fight"), or the row's own count (Holy Shield's blocks, Reckoning's extra
 * attacks), and a row whose threat is mana it gave you says that mana (Shield Specialization). Then
 * the crit, avoided and glancing shares of its attempts, left out for a row that can neither crit nor
 * be avoided (Holy Shield's damage). A bleed's row counts applications and ticks apart: crits from
 * its ticks, avoidance from its applications, and then, on a line of its own, its uptime on the
 * boss, with its average stacks for one that stacks (Lacerate). A spell on the boss (Faerie Fire)
 * can't crit, so it gives only its share missed. On the Damage metric the line ends with the average
 * damage per landed hit, landed ticks for a bleed's or a periodic effect's row: its damage over its
 * hits, crits, glances and blocks.
 */
export function outcomeLines(a: AbilityResult, fights: number, damageMetric: boolean): OutcomeLines {
  ...
  if (a.unit && fights > 0) parts.push(`${formatOne((a.unit === 'ticks' ? landed + avoided : a.casts) / fights)} ${a.unit} a fight`)
  ...
  if (a.certain) {
    ...
    if (a.bleed.avoidable && a.casts > 0) parts.push(`${formatPct((100 * avoided) / a.casts)} of ${a.unit === 'procs' ? 'procs' : 'applications'} avoided`)
  ...
  const average =
    damageMetric && a.damage > 0 && landed > 0 ? { value: formatInt(a.damage / landed), per: a.bleed || a.unit === 'ticks' ? ('tick' as const) : ('hit' as const) } : null
