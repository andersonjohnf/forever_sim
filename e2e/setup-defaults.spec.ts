...
test.describe('defaults', () => {
  ...
  test('a changed Fight setting shows its default, and its Reset puts it back and keeps focus on it', async ({ page }) => {
    ...
    expect(await hitArea44(tab.getByRole('button', { name: 'Reset Fight length, default 3:00' }))).toEqual([])
    await tab.getByRole('button', { name: 'Reset Fight length, default 3:00' }).click()
    ...
    await tab.getByRole('button', { name: 'Reset Position, default Behind' }).click()
  ...
  test('Fight’s Advanced opens by itself when a setting in it is changed, and counts them', async ({ page }) => {
    ...
    await tab.getByRole('button', { name: 'Reset Boss level, default 63' }).click()
  ...
  test('Character: a changed race and rules show their defaults, and reset', async ({ page }) => {
    ...
    await tab.getByRole('button', { name: 'Reset Race, default Human' }).click()
    ...
    await expect(tab.getByRole('button', { name: 'Advanced, 1 changed' })).toBeVisible()
    await tab.getByRole('button', { name: 'Reset Rules, default Forever' }).click()
