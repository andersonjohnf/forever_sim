...
| Damage snapshot | **Both profiles (default):** AP and the caster's damage modifiers are fixed when the DoT is applied; target-side modifiers apply per tick [?]. **Exceptions:** `forever`'s Rend adds 0.02 × AP to each tick, read as the tick lands ([D36](../decisions.md#d36-what-we-take-from-warriorsim-2026-09-25)); Deep Wounds is a rolling pool in `forever` (each crit's amount snapshotted at the crit, [D36](../decisions.md#d36-what-we-take-from-warriorsim-2026-09-25)) and recomputes each tick from current AP and modifiers in `classicEra` [C] (pre-SoD WarriorSim `DeepWounds.step`, [ws-spell]); see [warrior §2.5, §3.1](../classes/warrior.md#25-crits-impale-flurry-deep-wounds) | The snapshot default is [?]: the only Classic code for it (WarriorSim's Rend) is post-SoD, and no Classic Era measurement was found. `forever`: the same tzcnt notes report Rend reading AP, modifiers and crit per tick (~0.02 × AP per tick at level ~10) [?]; D36 adopts only the attack-power term; see [Open questions](#open-questions) |
...
3. **DoT snapshot and refresh rules** [?]. Both profiles snapshot AP, caster modifiers and crit
   ...
   tick** ([tz-forever]; anecdotal); [D36](../decisions.md#d36-what-we-take-from-warriorsim-2026-09-25) adopts only its 0.02 × AP a tick, read as it lands. Is there any pandemic-style carry-over? Do Rake, Rip, Pounce and Lacerate behave the
