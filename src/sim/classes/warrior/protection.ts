...
/** A duty row's place (rows 1, 5 and 6): D26's rule, not a preference, so it's pinned (D31). */
const DUTY_PLACE = 'Its place is the tank duties’ rule: before any threat ability on the global cooldown, so it can’t be moved.'
 ...
 * and the trinkets. The pre-pull is pinned first, and the duties, Shield Block, Thunder Clap and
 * Demoralizing Shout, are pinned where D26's rule puts them, before any threat ability on the global
 * cooldown. The Priority choice is spec-wide and is the list's preset picker (`presetChoice`): its
 * three values are the presets, and they share the default order. The consumables (row 4) are
 * spec-wide, above the list; they keep their place in it, after the off-GCD cooldowns and before the
 * pinned Thunder Clap.
 */
export const PROTECTION_APL: AplDefinition = {
  ...
  specWide: [ID.priority, ID.potionEnabled, ID.potionMaxRage, ID.jujuEnabled],
  presetChoice: ID.priority,
  presets: [
    {
      id: 'defensive',
      label: 'Defensive',
    ...
    },
    {
      ...
      values: { [ID.priority]: PROTECTION_PRIORITY.balanced },
    },
    {
      id: 'maxTps',
      label: 'Max TPS',
...
export function protectionRotation(
  ...
  // has them: after the off-GCD cooldowns (rows 1–3), before the pinned Thunder Clap.
  const consumables = () => {
  ...
  compileAplRows(PROTECTION_APL, order, {
    ...
    trinkets: () => trinketLines(b, v, ID, ctx, { dw: -1, align: false }),
