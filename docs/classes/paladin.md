...
Toughness or any other filler. That fill order is all it is: the search's leader is the answer
...
| --- | --- | --- |
| Anticipation (5) | "Increases your Defense Skill by 20." [F] ([Protection tree](#protection-tree)) | Defense: fewer crits and more misses, dodges, parries and blocks against the boss. The sim measures what its avoided hits cost in mana and Reckoning procs and nothing of what they save, so it's first in the fill order: leftover points go to it before Toughness ([optimizer](../optimizer.md#the-preferred-filler)). A build whose points gain more elsewhere may still drop it (user decision, D30) |
