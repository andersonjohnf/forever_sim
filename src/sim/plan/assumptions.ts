...
const REGISTRY = {
  ...
  foreverBossParry: {
    text: 'From the front the boss parries 16.5% of attacks, as the Forever client shows (Classic Era: 14%); unmeasured against a raid boss.',
...
export const BEAR_TEXT = {
  ...
  threat(uses: { maul: boolean; swipe: boolean; mangle: boolean; lacerate: boolean; faerieFire: boolean; roar: boolean }, profile: RuleProfileId = 'forever'): string {
    ...
    const sentences = [
      ...
      uses.maul
        ? 'Only Maul’s has been tested in Forever: the user’s in-game test at level 12 put it between 1.71 and 2.25 threat per damage, which 1.75 fits.'
