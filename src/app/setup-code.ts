...
import { CONFIG_VERSION, isReadableVersion, normalizeConfig, SPEC_IDS, SPEC_META, type SimConfig, type SpecId } from '@/sim'
...
export function setupProblem(raw: unknown): SetupProblem | null {
  ...
  if (!isReadableVersion(version)) return typeof version === 'number' && version > CONFIG_VERSION ? 'newer' : 'notASetup'
