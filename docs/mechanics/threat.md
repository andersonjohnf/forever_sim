...
| **Lacerate** (Forever, r3 1235827, trained 42/50/58) | — | **dmg × 1 + 206** per landed application [?]; ticks dmg × 1 | 15 rage; bleeds 75 over 15 s, stacks to 5 [F]. The tooltip says "Causes a high amount of threat", with no threat effect in the client, so the bonus is the [wording table](#threat-wording-table)'s: Forever's Sunder Armor r5 at the same level 58, its client value 206 since 1.60.1.70009, flat (user decision, 2026-09-26); Classic Era's rule, 4.5 × level = 261, until that build. **Engine:** 206 × the global multipliers on each landed application (a hit or a block), the first one too, which deals no damage; none for a miss, dodge or parry. `classicEra`: 261. Season of Discovery's Lacerate is the same spell (Forever's rank 1 reuses its id, 414644), and Blizzard's SoD hotfixes of 2 December 2024 give its hit and ticks 3.33 × damage (was 3.5) ([SoD hotfixes](https://us.forums.blizzard.com/en/wow/t/season-of-discovery-hotfixes/1788040)): an allowed analog (D36, D37), not adopted. An in-game test (druid.md G1) replaces the default. |
