...
describe('a shared link’s setup', () => {
  ...
  test('a newer version’s link says to reload, as a newer file does', () => {
    const read = readLinkSetup({ ...fresh('warrior-arms'), version: 3 })
  ...
  test('a spec the sim doesn’t know, or doesn’t offer yet, says so', () => {
    expect(readLinkSetup({ version: 1, spec: 'mage-spellblade' })).toMatchObject({ ok: false, title: 'That link is for a spec this sim doesn’t know' })
