...
Blizzard's SoD data for a reused spell, client data, a measurement), tagged [?] and shown in the
assumptions; an undescribed client dummy models as zero, tagged [?] and listed as an open
question. Other sims and the user's offhand numbers are never evidence, and no value moves to
close a gap. Treat the same threat wording the same on every tank, the bonus used as is; gear
