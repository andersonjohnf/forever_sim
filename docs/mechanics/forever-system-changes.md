...
| **Rage normalization** | Swing rage is a fixed amount per landed hit set by weapon speed. [rage.md](rage.md#forever-normalized-rage-per-swing-) owns the model: `k × speed` with k = 3.5 (1H) or 4.5 (2H), no crit bonus [?]; one third-party log set fits 3.46 for 1H ([fw-3]). Damage-taken rage is `10 × damage before armor, block and absorbs ÷ max health` [?], fitted to about 2,000 logged beta hits: blocked and absorbed hits give full rage, several attackers each count, and it is much lower than Classic at 60 ([rage § damage taken](rage.md#forever-)). | [?] third-party beta logs ([magey/forever-warrior#3][fw-3]; [wowsims/forever#2][wf-2]; [tzcnt/forever-data][fd-logs]) | **high** (warrior, bear) | [rage.md](rage.md) |
